...
import { commentApi, fetchFromAPI, getVideoIdApi, likeApi } from "../utils/fetchFromAPI";
...
const VideoDetail = () => {
  ...
  if (videoDetail) {
    ...
    const { video_name, description, source, views, user, video_id } = data;
                    ...
                    <div className="card-footer py-3 border-0 text-white" style={{ backgroundColor: '#000' }}>
                      ...
                      <div className="float-end mt-2 pt-1">
                        <button type="button" className="btn btn-outline-light btn-sm me-3"
                          ...
                          onClick={() => {
                            let videoId = video_id;
                            let content = document.querySelector("#textAreaExample").value;

                            commentApi({ videoId, content }).then(result => {
